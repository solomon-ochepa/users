...
namespace Modules\OAuth\app\Providers;
...
class RouteServiceProvider extends ServiceProvider
{
     ...
     * @var string
     */
    protected $moduleNamespace = 'Modules\OAuth\app\Http\Controllers';
    ...
    {
        // Tenant route (web)
        if (file_exists($tenant_web_route = module_path('OAuth', '/routes/web.tenant.php'))) {
            Route::middleware('web')->namespace($this->moduleNamespace)->group($tenant_web_route);
        }

        // Central domains route (web)
        $central_domains    = config('tenancy.central_domains');
        $web_route          = module_path('OAuth', '/routes/web.php');
    ...
    {
        // Tenant route (api)
        if (file_exists($tenant_api_route = module_path('OAuth', '/routes/api.tenant.php'))) {
            Route::prefix('api')->middleware('api')->namespace($this->moduleNamespace)->group($tenant_api_route);
        }

        // Central domains route (api)
        $central_domains    = config('tenancy.central_domains');
        $api_route          = module_path('OAuth', '/routes/api.php');
